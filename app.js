...
function renderTopScorers() {
  ...
  $('#app').innerHTML = `<section class="section"><div class="wrap"><div class="title"><h2>Top Scoring Teams</h2><span class="tag">Highest scoring team award view</span></div><div class="topscorer-showcase"><div class="leader-card">${leaderLogoBlock(leader.team)}<div class="leader-label">Leading team</div><div class="leader-rank">#${leader.rank}</div><h3>${escapeHtml(leader.team)}</h3><div class="leader-goals">${leader.GF}</div><p class="small">Goals scored</p><div class="leader-meta"><span class="tag">Table position: ${leader.standingRank}</span><span class="tag">Points: ${leader.Pts}</span><span class="tag">Goal difference: ${leader.GD}</span></div></div><div class="card topscorer-side-list"><div class="small-list-heading">Top 5 scoring teams</div>${sideRows.map((row) => `<div class="topscorer-item ${row.rank === 1 ? 'is-leading' : ''}"><div class="topscorer-rank">#${row.rank}</div>${teamLogoHtml(row.team, 'topscorer-list-logo')}<div class="topscorer-team"><b>${escapeHtml(row.team)}</b><span class="small">P ${row.P} • W ${row.W} • D ${row.D} • L ${row.L}</span></div><div class="topscorer-goals">${row.GF}<span>goals</span></div></div>`).join('')}</div></div><div class="table-scroll top-scorer-table-wrap" style="margin-top:22px"><table class="table top-scorer-table"><tr><th class="rank-col">Rank</th><th class="team-col">Team</th><th class="number-col">Goals Scored</th><th class="number-col">Matches</th><th class="number-col">Points</th><th class="number-col">GD</th></tr>${rows.map((row) => `<tr><td class="rank-col"><b>#${row.rank}</b></td><td class="team-col"><div class="table-team-cell">${teamLogoHtml(row.team, 'table-team-logo')}<b>${escapeHtml(row.team)}</b></div></td><td class="number-col">${row.GF}</td><td class="number-col">${row.P}</td><td class="number-col">${row.Pts}</td><td class="number-col">${row.GD}</td></tr>`).join('')}</table></div></div></section>`;
}
